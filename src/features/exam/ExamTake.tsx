...
export default function ExamTake() {
  ...
  const handleRunCode = async () => {
    ...
    if (!sampleTestCases || sampleTestCases.length === 0) {
        alert("هیچ تست نمونه‌ای برای این سوال وجود ندارد.");
        return;
    }
    ...
    try {
        ...
        setExecutionResults([{ status: 'error', actualOutput: 'خطا در ارتباط با سرور' }]);
    } finally {
          ...
          <div className="flex-grow">
            <div className="bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-lg">
              ...
              <div className="mb-8">
                ...
                {currentQuestion.mediaUrl && renderMedia(currentQuestion.mediaUrl)}
                            ...
                            return (
                                <div className="space-y-4">
                                    <Editor
                                        ...
                                        onValueChange={code => handleAnswerChange(currentQuestion.id, code)}
                                        highlight={code => highlight(code, currentQuestion.language === 'python' ? languages.python : languages.js, currentQuestion.language || 'js')}
                                        padding={10}
                                        className="bg-gray-900 text-white font-mono rounded-lg border border-gray-700 min-h-[300px]"
                                    />
                                    <Button onClick={handleRunCode} disabled={isExecuting}>
                                        {isExecuting ? <Spinner size="sm" /> : 'اجرای تست‌ها'}
                                    </Button>
                                    {executionResults && (
                                        <div className="border-t pt-4">
                                            <h4 className="font-bold mb-2">نتایج اجرا:</h4>
                                            {executionResults.map((result, i) => (
                                                <div key={i} className={`p-2 rounded mb-2 ${result.status === 'success' ? 'bg-green-100' : 'bg-red-100'}`}>
                                                    <p>تست {i + 1}: <span className="font-bold">{result.status}</span></p>
                                                    <pre className="whitespace-pre-wrap">ورودی: {result.input}</pre>
                                                    <pre className="whitespace-pre-wrap">خروجی شما: {result.actualOutput}</pre>
                                                    {result.status !== 'success' && <pre className="whitespace-pre-wrap">خروجی مورد انتظار: {result.expectedOutput}</pre>}
                                                </div>
                                            ))}
                                        </div>
                                    )}
