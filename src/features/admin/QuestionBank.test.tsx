...
describe('QuestionBank', () => {
  ...
  it('should open the add question modal and dynamically show inputs', async () => {
    ...
    // Change to essay-with-upload
    fireEvent.change(typeSelector, { target: { value: 'essay-with-upload' } });

    // No options or correct answer fields should be visible
    expect(screen.queryByLabelText('پاسخ صحیح (جای خالی)')).toBeNull();
    expect(screen.queryByText('گزینه‌ها و پاسخ(های) صحیح')).toBeNull();
  });
});
