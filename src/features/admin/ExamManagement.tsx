...
export default function ExamManagement() {
  ...
  const handleFormSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const examData = {
      ...
      // We will fetch the full question objects on the backend based on IDs
      questions: selectedQuestionIds.map(id => ({ id } as Question)),
  ...
  const deleteExam = async (id: string) => {
    if (window.confirm('آیا از حذف این آزمون مطمئن هستید؟')) {
      try {
        ...
        <Modal isOpen={isModalOpen} onClose={closeModal} title={modalMode === 'add' ? 'افزودن آزمون جدید' : 'ویرایش آزمون'} size="3xl">
            <form onSubmit={handleFormSubmit} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <Input label="تاریخ شروع" name="startDate" type="datetime-local" defaultValue={currentExam?.startDate} />
                    <Input label="تاریخ پایان" name="endDate" type="datetime-local" defaultValue={currentExam?.endDate} />
                ...
                <div className="border-t pt-6">
                  <h3 className="text-lg font-bold">انتخاب سوالات</h3>
                  <p className="text-sm text-gray-500 mb-4">
                    سوالات مورد نظر خود را از بانک سوالات انتخاب کنید. ({selectedQuestionIds.length} سوال انتخاب شده)
                  </p>
                  <QuestionSelector
