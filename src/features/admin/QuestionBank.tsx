...
export default function QuestionBank() {
  ...
  const openModal = (mode: 'add' | 'edit', question: Question | null = null) => {
    ...
    setSelectedQuestionType(question?.type || 'multiple-choice');
    setIsModalOpen(true);
  };
  ...
  const handleFormSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    let correctAnswer: number | number[] | string | undefined;
    ...
    if (questionType === 'fill-in-the-blank') {
        ...
        correctAnswer = [];
        for (let i = 0; i < 4; i++) {
            if (formData.get(`correctAnswer-${i}`)) {
    ...
    const questionData: Partial<Question> = {
        ...
        options: options,
        gradingCriteria: formData.get('gradingCriteria') as string || undefined,
    };
        ...
        <Modal isOpen={isModalOpen} onClose={closeModal} title={modalMode === 'add' ? 'افزودن سوال' : 'ویرایش سوال'}>
            <form onSubmit={handleFormSubmit} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <select name="type" aria-label="Question Type" value={selectedQuestionType} onChange={(e) => setSelectedQuestionType(e.target.value as QuestionType)} className="w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white" required>
                ...
                {selectedQuestionType === 'essay-with-upload' && (
                    <textarea name="gradingCriteria" placeholder="معیارهای ارزیابی را اینجا وارد کنید..." rows={3} className="w-full p-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600" defaultValue={currentQuestion?.gradingCriteria}></textarea>
                )}

                {selectedQuestionType === 'fill-in-the-blank' ? (
                    <div>
                        <Input name="correctAnswer" label="پاسخ صحیح (جای خالی)" defaultValue={typeof currentQuestion?.correctAnswer === 'string' ? currentQuestion.correctAnswer : ''} required />
                    </div>
                ) : selectedQuestionType !== 'essay-with-upload' ? (
                    <>
                        <h3 className="font-semibold pt-2 border-t">گزینه‌ها و پاسخ(های) صحیح</h3>
                        {Array.from({ length: 4 }).map((_, i) => (
                            <div key={i} className="flex items-center">
                                {selectedQuestionType === 'multiple-choice' ? (
                                    ...
                                    <input type="checkbox" name={`correctAnswer-${i}`} value={i} defaultChecked={Array.isArray(currentQuestion?.correctAnswer) && currentQuestion.correctAnswer.includes(i)} className="ml-2" />
                                )}
                                <Input name={`option-${i}`} placeholder={`گزینه ${i + 1}`} defaultValue={currentQuestion?.options?.[i]} required />
                            </div>
                        ))}
                    </>
                ) : null}
